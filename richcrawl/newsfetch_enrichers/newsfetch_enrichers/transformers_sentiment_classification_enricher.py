...
class TransformerSentimentClassificationEnricher(TransformersEnricher):
    def __init__(self, model_name: str = "distilbert-base-uncased-finetuned-sst-2-english"):
        super().__init__(model_name)
        self.classifier_pipeline = pipeline(model=self.model_name, truncation=True)

    def enrich(self, text: str):
        results = self.classifier_pipeline(text)
        return {"sentiment": results[0]["label"], "sentiment_score": results[0]["score"]}
