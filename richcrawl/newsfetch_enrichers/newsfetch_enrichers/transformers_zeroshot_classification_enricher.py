...
from richcrawl.newsfetch_enrichers.common.datatypes import Classifications, Classification
from richcrawl.newsfetch_enrichers.newsfetch_enrichers.transformers_enricher import TransformersEnricher


class TransformersZeroShotClassificationEnricher(TransformersEnricher):
    def __init__(self, model_name: str, candidate_labels: list, threshold: float = 0.75):
        super().__init__(model_name)
        self.candidate_labels = candidate_labels
        self.threshold = threshold
        self.classifier_pipeline = pipeline(config.TRANSFORMERS_ZERO_SHOT_CLASSIFICATION_PIPELINE_NAME,
                                            model=self.model_name,  truncation=True)
    ...
    def enrich(self, text: str) -> Classifications:
        ...
        # print(results)
        return results["labels"][scores.index(max(scores))]
...
if __name__ == '__main__':
    ...
    zeroshot_classification_enricher = TransformersZeroShotClassificationEnricher(
        config.TRANSFORMERS_CLASSIFICATION_VALHALLA_DISTILBART_MNLI_12_1,
